...
const currentTrackSlice = createSlice({
  ...
  initialState,
  reducers: {
    addTrackId(state: InitialCurrentTrackStateTypes, actions: PayloadAction<string>) {
      state.trackId = actions.payload
    },
    addTrackIndex(state: InitialCurrentTrackStateTypes, actions: PayloadAction<number>) {
      state.index = actions.payload
    }
  }
})
export const { addTrackId, addTrackIndex } = currentTrackSlice.actions
export const tunifyCurrentTrack = (state: RootState) => state.persistedReducer.currentTrack
