  ...
  return (
    <View className='bg-[#181a20] w-full h-screen '>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          return (
            <TouchableOpacity className='w-full h-16  mt-1 flex items-center flex-row px-2'>
              <Image source={item.leftIcon} style={{ tintColor: "#d0d0d1" }} className='h-7 w-7' />
              <Text className='ml-4 text-gray-300 text-base font-semibold tracking-widest'>{item.title}</Text>
